...
public class SimpleHelvetica : MonoBehaviour {
	...
	public void AddBoxCollider(){
		...
		//	lastChild.position.x + lastChild.GetComponent<BoxCollider>().size.x, 
		//	lastChild.position.y + lastChild.GetComponent<BoxCollider>().size.y/2, 
		//	lastChild.position.z + lastChild.GetComponent<BoxCollider>().size.z/2);
		//Vector3 middle = (transform.position + actualPosition) / 2;
		//Vector3 diff = middle - transform.position;
		//Debug.Log(middle);
		////Debug.Log(lastChild.position);
		//gameObject.AddComponent<BoxCollider>();
		//gameObject.GetComponent<BoxCollider>().center = diff;
		...
		foreach (Transform child in transform)
		{
			if (child.name != "_Alphabets")
			{
				if (rend.extents == Vector3.zero)
					rend = child.gameObject.GetComponent<MeshRenderer>().bounds;
